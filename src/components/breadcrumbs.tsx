...
const renderBreadcrumbItem = (el: Item, isLastItem: boolean = false, navigate: NavigateFunction) => {
  ...
  const buttonProps = {
    ...
    className: "!text-lg data-[first]:!font-semibold data-[first]:!text-xl font-medium dark:data-[first]:!text-blue-400 data-[first]:!text-blue-700",
  ...
  return (
    <React.Fragment key={`button-items-${el.key}`}>
      {isTruncatableBreadcrumbContent(el.item ?? '', 20) ? (
        <BreadcrumbItem>
          <Tooltip withArrow content={capitalizeText(el.item ?? '')} relationship="label">
            <BreadcrumbButton
              {...buttonProps}
              current={isLastItem}>
        ...
        <BreadcrumbItem>
          <BreadcrumbButton  {...buttonProps} current={isLastItem}>{el.item}</BreadcrumbButton>
        </BreadcrumbItem>
      )}
...
const OverflowMenu = (props: PartitionBreadcrumbItems<Item> & {
  ...
  const { overflowItems, startDisplayedItems, endDisplayedItems, navigate } = props;
            ...
            {isOverflowing &&
              startDisplayedItems.map((item: Item) => (
                <MenuItem
                  ...
                  id={item.key.toString()} key={item.key} >
                  {item.item}
                </MenuItem>
              ))}
            {overflowItems &&
