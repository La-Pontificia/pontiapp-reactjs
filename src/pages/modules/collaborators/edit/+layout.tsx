...
export default function CollaboratorsEditLayout(): JSX.Element {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await api.get<User>(
        'users/' +
          slug +
          '?relationship=role,role.department,role.department.area,userRole,manager'
      )
  ...
  if (isLoading)
    return (
      <div className="p-20 text-center h-full grid place-content-center">
        <Spinner size="huge" />
      </div>
    )

  if (!user)
    return <div className="p-20 text-center">No se encontró el usuario</div>
    ...
    <div className="relative h-full w-full flex flex-col overflow-y-auto">
      ...
      <nav className="flex gap-2 w-full items-center">
        <TabList
          ...
          onTabSelect={(_, d) =>
            navigate(`/modules/collaboradores/edit/${params.slug}/${d.value}`)
          }
