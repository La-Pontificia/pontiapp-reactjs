...
export default function UserTeamMemberItem({
  ...
  return (
    <tr className="relative even:bg-stone-500/20 [&>td]:text-nowrap transition-colors group [&>td]:p-2.5 [&>td]:px-3 bg-stone-900 first:[&>td]:first:rounded-tl-xl last:[&>td]:first:rounded-tr-xl first:[&>td]:last:rounded-bl-xl last:[&>td]:last:rounded-br-xl">
      ...
      <td>
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            to={`/modules/collaborators/${member.user.username}`}
          >
      ...
      <td className="">
        <UserGridOptions refetch={refetch} member={member} />
      </td>
    </tr>
  )
}
...
export const UserGridOptions = ({
  ...
  const handleRemoveMember = async () => {
    ...
    const res = await api.post<string>(
      `partials/teams/members/remove/${member.id}`
    )
    if (!res.ok) {
  ...
  return (
    <>
      <Menu hasIcons positioning={{ autoSize: true }}>
        ...
        <MenuPopover>
          <MenuList>
            <MenuItem onClick={() => setIRemoveAlertOpen(true)}>
              Remover del equipo
            </MenuItem>
          </MenuList>
        </MenuPopover>
