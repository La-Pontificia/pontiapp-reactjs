...
export default function PeriodForm({
  ...
  const { data, isLoading, refetch } = useQuery<RmData[]>({
    queryKey: ['periods', 'rm', q],
    queryFn: async () => {
      const res = await api.get<RmData[]>(`rm/periods?q=${q ? q : ''}`)
      if (!res.ok) {
        return []
      }
  ...
  const { mutate: create } = useMutation({
    mutationKey: ['periods', 'rm', 'create', q],
    mutationFn: () =>
      api.post<RmData>('rm/periods/store', {
