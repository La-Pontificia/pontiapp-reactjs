...
export default function CycleForm({
  ...
  const { data, isLoading, refetch } = useQuery<RmData[]>({
    queryKey: ['cycles', 'rm', q],
    queryFn: async () => {
      const res = await api.get<RmData[]>(`rm/cycles?q=${q ? q : ''}`)
      if (!res.ok) {
        return []
      }
  ...
  const { mutate: create } = useMutation({
    mutationKey: ['cycles', 'rm', 'create', q],
    mutationFn: () =>
      api.post<RmData>('rm/cycles/store', {
