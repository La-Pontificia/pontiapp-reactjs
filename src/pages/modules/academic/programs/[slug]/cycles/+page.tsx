...
export default function CyclesPage() {
  ...
  const { data, isLoading, refetch } = useQuery<ResponsePaginate<
    Cycle[]
  > | null>({
    queryKey: ['academic/cycles', filters, program.id],
    queryFn: async () => {
      const res = await api.get<ResponsePaginate<Cycle[]>>(
        'academic/cycles' + query
      )
      if (!res.ok) return null
      return {
        ...res.data,
        data: res.data.data.map((item) => new Cycle(item))
      }
    }
  })
